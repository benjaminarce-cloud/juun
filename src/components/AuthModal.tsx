...
export default function AuthModal() {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} aria-label="Cuenta" style={{
        background:'none',border:'none',cursor:'pointer',
        padding:'0.25rem',color:'inherit',display:'flex',alignItems:'center',
      }}>
      ...
      {open && (
        <div style={{
          ...
          <button onClick={() => setOpen(false)} style={{
            ...
            background:'none',border:'none',cursor:'pointer',
            fontSize:'1.1rem',color:'rgba(14,12,11,0.35)',lineHeight:1,
          }}>✕</button>

          <div style={{marginBottom:'2rem'}}>
            <span style={{fontSize:'1.1rem',fontWeight:900,letterSpacing:'-0.03em'}}>JÜUN</span>
            <span style={{fontSize:'0.55rem',letterSpacing:'0.15em',marginLeft:'0.4rem',opacity:0.4,textTransform:'uppercase' as const}}>wellness</span>
          </div>
          ...
          <div style={{display:'flex',marginBottom:'2rem',borderBottom:'1px solid rgba(14,12,11,0.1)'}}>
            {(['login','signup'] as const).map(m => (
              <button key={m} onClick={() => setMode(m)} style={{
                ...
                fontSize:'0.65rem',fontWeight:mode===m?700:400,
          ...
          {mode==='login' && (
            <p style={{textAlign:'center' as const,marginTop:'1rem',fontSize:'0.65rem',opacity:0.35,cursor:'pointer'}}>
              ¿Olvidaste tu contraseña? <span style={{textDecoration:'underline'}}>Recuperar</span>
            </p>
          )}

          <p style={{textAlign:'center' as const,marginTop:'2rem',fontSize:'0.58rem',opacity:0.25,letterSpacing:'0.08em',textTransform:'uppercase' as const}}>
